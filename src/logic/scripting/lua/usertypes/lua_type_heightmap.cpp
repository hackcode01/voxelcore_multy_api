...
LuaHeightmap::LuaHeightmap(uint_t width, uint_t height)
...
uint_t LuaHeightmap::getWidth() const {
    return map->getWidth();
}

uint_t LuaHeightmap::getHeight() const {
...
static int l_dump(lua::State* L) {
    if (auto heightmap = touserdata<LuaHeightmap>(L, 1)) {
        ...
        uint_t w = heightmap->getWidth();
        uint_t h = heightmap->getHeight();
...
static int l_resize(lua::State* L) {
    if (auto heightmap = touserdata<LuaHeightmap>(L, 1)) {
        uint_t width = touinteger(L, 2);
        uint_t height = touinteger(L, 3);
...
static int l_crop(lua::State* L) {
    if (auto heightmap = touserdata<LuaHeightmap>(L, 1)) {
        uint_t srcX = touinteger(L, 2);
        uint_t srcY = touinteger(L, 3);

        uint_t dstWidth = touinteger(L, 4);
        uint_t dstHeight = touinteger(L, 5);
...
static int l_meta_meta_call(lua::State* L) {
    ...
    return newuserdata<LuaHeightmap>(
        L, static_cast<uint_t>(width), static_cast<uint_t>(height)
    );
}
