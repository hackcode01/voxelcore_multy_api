...
namespace util {
    ...
    std::wstring lfill(std::wstring s, uint_t length, wchar_t c);
    std::wstring rfill(std::wstring s, uint_t length, wchar_t c);

    uint_t encode_utf8(uint32_t c, ubyte* bytes);
    uint32_t decode_utf8(uint_t& size, const char* bytes);
